...
public:
  ...
  void ViewMessage(const std::string message, int vitesseDefilement = 100,
                   uint16_t colorText = BLUE, uint16_t colorBackground = BLACK);
  void ViewLetter(char lettre, uint16_t colorText = BLUE,
                  uint16_t colorBackground = BLACK);
...
private:
  ...
  void ConvertCharacterToPattern(char c, uint16_t image[8][8],
                                 uint16_t colorText, uint16_t colorBackground);
  bool EmptyColumn(int numcolumn, uint16_t image[8][8],
